...
import { ChartBarInteractive } from "@/components/dashboard/dashboard-escalation-bar-chart";
...
const dashboardData = {
  ...
  recentActivity: [
    ...
    {
      type: "escalation",
      message: "Case #123456 escalated to support",
      time: "15 min ago",
      status: "pending",
    },
    {
      type: "agent",
      message: "Agent Sarah went online",
      time: "1 hour ago",
      status: "online",
    },
    {
      type: "knowledge",
      message: "FAQ updated: Payment Issues",
      time: "2 hours ago",
      status: "updated",
    },
  ],
    ...
    const fetchAverageRating = async () => {
      ...
      try {
        ...
        const response = await api.get(
          `/analytics/business-ratings/${user.businessId}/average`
        );
  ...
  useEffect(() => {
    const fetchEscalatedCount = async () => {
      ...
      try {
        ...
        const response = await api.get(
          `/analytics/escalations/${user.businessId}/count`
        );
        if (response.data.success) {
  ...
  useEffect(() => {
    const fetchSessionsCount = async () => {
      ...
      try {
        ...
        const response = await api.get(
          `/analytics/sessions/${user.businessId}/count`
        );
        if (response.data.success) {
  ...
  useEffect(() => {
    const fetchAgentsCount = async () => {
      ...
      try {
        ...
        const response = await api.get(
          `/analytics/agents/${user.businessId}/count`
        );
        if (response.data.success) {
    ...
    <div className="w-full mx-auto p-6 space-y-8">
      ...
      {businessData._id && (
        <div className="grid gap-4 lg:grid-cols-12 mb-8">
          ...
          <div className="lg:col-span-9">
            <ChartBarInteractive businessId={businessData._id} />
          </div>
