...
interface UseChatWidgetLiveChatProps {
...
export function useChatWidgetLiveChat({
  waitingForAgent,
  escalationResponse,
  businessData,
  onChatStarted,
  ...
  const requestChat = (escalationId: string, businessId: string) => {
    if (socketRef.current && escalationId && businessId) {
      ...
      if (socket.connected) {
        console.log('[CHAT_WIDGET] Connected, emitting join_escalation and request_chat', {
          escalationId,
          businessId,
        ...
        socket.once('connect', () => {
          console.log('[CHAT_WIDGET] Connected (delayed), now emitting join_escalation and request_chat', {
            escalationId,
            businessId,
            socketConnected: socket.connected,
            socketId: socket.id
  ...
  useEffect(() => {
    return () => {
      if (
        ...
        console.log('[CHAT_WIDGET] Leaving queue due to component unmount or navigation')
  ...
  useEffect(() => {
    ...
    const handleChatStarted = (data: { agentId: string, escalationId: string, room: string }) => {
      console.log('[CHAT_WIDGET] Chat started:', data)
      onChatStarted(data)
    }

    // Agent connected event
    const handleAgentConnected = (data: { agentId: string, room: string, message: string }) => {
      console.log('[CHAT_WIDGET] Agent connected:', data)
      onAgentConnected?.(data)
    }

    // New message event
    const handleNewMessage = (data: any) => {
      console.log('[CHAT_WIDGET] New message received:', data)
      onNewMessage?.(data)
    }

    // System message event
    const handleSystemMessage = (data: any) => {
      console.log('[CHAT_WIDGET] System message received:', data)
      onSystemMessage?.(data)
    }

    // Agent disconnected during chat event
    const handleAgentDisconnectedDuringChat = (data: any) => {
      console.log('[CHAT_WIDGET] Agent disconnected during chat:', data)
      onAgentDisconnectedDuringChat?.(data)
    }

    // Agent typing events
    const handleAgentTyping = (data: any) => {
      console.log('[CHAT_WIDGET] Agent is typing:', data)
      onAgentTyping?.(data)
    }

    const handleAgentStoppedTyping = (data: any) => {
      console.log('[CHAT_WIDGET] Agent stopped typing:', data)
      onAgentStoppedTyping?.(data)
    }

    // Chat error event
    const handleChatError = (data: any) => {
      console.log('[CHAT_WIDGET] Chat error:', data)
      onChatError?.(data)
    }

    // Chat ended event
    const handleChatEnded = (data: any) => {
      console.log('[CHAT_WIDGET] Chat ended:', data)
      onChatEnded?.(data)
    }
