      ...
      try {
        ...
        if (response.data.success) {
          ...
          const qrData = {
            ...
            level: (settings.errorCorrectionLevel || "M") as
              | "L"
              | "M"
              | "Q"
              | "H",
            ...
            includeLogo:
              settings.includeLogo !== undefined ? settings.includeLogo : true,
          };
    ...
    try {
      ...
      if (qrSettings.includeLogo) {
        ...
        await new Promise((resolve, reject) => {
          ...
          const logoSrc = logoToUse.startsWith("http")
            ? logoToUse
            : `${window.location.origin}${logoToUse}`;
          logoImg.src = logoSrc;
        });
    ...
    <Card className="bg-card border-muted-gray shadow-none h-full flex flex-col">
      ...
      <CardContent className="flex-1 flex flex-col space-y-4 px-6 pb-6">
        ...
        <div className="flex flex-col items-center justify-center space-y-4">
          <div
            ref={qrRef}
            className="relative p-4 bg-white rounded-xl border border-border shadow-sm"
        ...
        <div className="space-y-2 pt-3 border-t border-border mt-4">
          ...
          <div className="flex gap-2">
            <Input
              readOnly
              value={chatUrl}
              className="bg-muted/50 shadow-none text-xs text-foreground border-muted-gray font-mono"
            />
            <Button
              onClick={handleCopy}
              variant="outline"
